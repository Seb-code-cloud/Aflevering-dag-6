...
# -----------------------------------------------
#Here we ask user to add a new customer
...
def Latest_added(result):
    ...
    *
	...
	table = BeautifulTable()						#Kan desværre ikke få alle Headers tilføjet
	table.columns.header = ["Customer info"]     #["Costumer_ID", "Email", "Reg_date", "First_name", "Last_name", "Phone_number", "Password", "Country", "Default_shipping_Addr"]
...
#der laves en variabel kaldet "title", som vi bruger i vores funktion "hello". 
title = "\t\t\tMENU\t\t\t"

def hello():
    print("\033[H\033[J") #dette stykke giver en overskuelig menu og fjerner alle andre ting i terminalen
    print(title)
    print("\n")
